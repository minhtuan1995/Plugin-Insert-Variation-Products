...
//    wp_set_object_terms($post_id, $product_data['categories'], 'product_cat'); // Set up its categories
    ...
    insert_product_variations($post_id, $product_data['variations'], $attachments); // Insert variations passing the new post id & variations   
    //Update tags
...
function insert_product_variations ($post_id, $variations, $attachments)  
{
    foreach ($variations as $index => $variation)
    {
        ...
        if (isset($variation['image']) && !empty($variation['image'])) {
           ...
           if (isset($attachments[$variation['image'][0]['id']])) {
                ...
                add_post_meta($variation_post_id, '_thumbnail_id', $attachment_id);  
            }
...
function attach_image ($file, $filealt, $post_id)  
{
    $fileurl = $file['src'];
